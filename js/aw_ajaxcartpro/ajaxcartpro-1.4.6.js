...
function showProgressAnimation(){
	...
	// UFFA
	$j('.ajaxcartpro_progress').hide();	
	$j('.ajaxcartpro_progress').fadeIn();	
}
...
function showConfirmDialog(product_name){
	...
	// UFFA
	$j('.ajaxcartpro_confirm').hide();	
	$j('.ajaxcartpro_confirm').fadeIn();
    ...
    document.observe("dom:loaded", function() {
        ...
        if($('aw_acp_checkout')) {
            $('aw_acp_checkout').onclick = function(e){
				// UFFA
				setTimeout(function() { $j('.ajaxcartpro_confirm').fadeOut() }, 6000);
//                $$('.ajaxcartpro_confirm')[0].style.display='none';
                return true;
            }
        }
