    ...
    extends Mage_Core_Helper_Data
{
    ...
    public function sendEmail($data)
    {
         ...
         */
        if ($gc->getData('send_on') && $gc->getData('send_on') != date('Y-m-d')) {
            $this->setSentSuccess(false);
            Mage::log($this->__("GC '%s' is scheduled on '%s' and cannot be sent on '%s'.",
                    $gc->getData('cert_number'), $gc->getData('send_on'), date('Y-m-d')),
...
//            $template = Mage::getStoreConfig($self ? 'ugiftcert/email/template_self' : 'ugiftcert/email/template', $store);
    ...
    public function sendOrderItemEmail($item, $giftcerts = null)
    {
        if(!$item || !$item instanceof Mage_Sales_Model_Order_Item){
            Mage::getSingleton('adminhtml/session')->addError($this->__("Missing order item. Email not sent. Or item not instance of Mage_Sales_Model_Order_Item class."));
    ...
    public function sendGiftcertEmails(array $certIds)
    {
        ...
        return $this->sendCollection($giftcerts);
    }
     ...
     * @return array
     */
    public function sendCollection($collection)
    {
        $grouped = array();
        $emailsSent = 0;
        $certsSent = 0;
