    ...
    */
    ...
    public function sales_order_payment_place_start($observer)
    {
        ...
        if ($baseTotalAmount>0 && $order->getBaseGrandTotal()==0) {
            Mage::throwException(Mage::helper('ugiftcert')->__('Gift certificates applied to this order have changed. Unable to proceed, please return to shopping cart to see the changes.'));
        }
    }
...
//                ->addFieldToFilter('cert_number', array('in' => $certIds));
            $select = $collection->getSelect()->reset(Zend_Db_Select::COLUMNS)->columns('cert_number')->where('cert_number in (?)', $certIds);
            $select->join(array('h'=>$collection->getTable('ugiftcert/history')), 'h.cert_id=main_table.cert_id', array('ts', 'amount', 'customer_email', 'currency_code'))
    ...
    */
    public function controller_front_init_before($observer)
    {
        if (version_compare(Mage::getVersion(), '1.3.0', '<')) {
            Mage::getConfig()->setNode('frontend/events/controller_action_predispatch_checkout/observers/ugiftcert/method', 'apply_quote_item_to_products');
...
//        $product->setData('ugiftcert_pdf_settings', $hlp->loadPdfSettings());
    ...
    public function sendScheduledCertificates()
    {
        ...
        if ($collection->count()) {
            ...
            Mage::log("%d certificates sent", Zend_Log::INFO, Unirgy_Giftcert_Helper_Data::LOG_NAME, $collection->count());
        } else {
