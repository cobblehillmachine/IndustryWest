...
class Fooman_OrderManager_Block_Adminhtml_Widget_Grid_Column_Renderer_PaymentDetails extends Mage_Adminhtml_Block_Widget_Grid_Column_Renderer_Select
{
    public function render(Varien_Object $row)
    {
		...
		//print_r($row);
//		$payment = Mage::getModel('sales/order_payment')->load($row->getPaymentId());
//		print_r($method->getTitle());
        ...
        if ($trackingCarriers) {
            ...
            $html = '<select style="width:60px;" name="'.$colId.'-'.$row->getId().'" rel="'.$row->getId().'" class="'.$colId.'" >';
    ...
    public function getOptions($row)
    {
        ...
        if (isset ($carriers['custom']) && $customTitle = Mage::helper('ordermanager')->getStoreConfig('settings/customtitle')) {
