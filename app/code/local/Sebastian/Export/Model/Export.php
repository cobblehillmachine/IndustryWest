...
//$end = 0;
	  ...
	  if (isset($start)) {
  		  if ((int)$start != 0) {
		  	$start = (int)$start - 100000000;
		  }
	  }

	  if (isset($end)) {
		  if ((int)$end != 0) {
			   $end = (int)$end - 100000000;
		  }
	  }*/
...
$p_string = implode($p,""); */
//Mage::throwException(Mage::helper('export')->__("condition" .$start . "-". $end. "-" . count($condition). "-" . $p_string));          
...
//$collection->addFieldToFilter('status', array('nin' => array('complete', 'canceled', 'closed')));
      ...
      if ($export_type == 'xml' || $export_type == 'csv' || $export_type == 'custom') {
        ...
        if (!$xw->openMemory()) {
          ...
          foreach ($collection as $order) {
            $ordercount++;
//            $lastOrderId = (int)$order->getData('entity_id')- 100000000; 
  //          $realOrderId = (int)$order->getData('increment_id')- 100000000 -1 ; // last exported id
            $lastOrderId = (int)$order->getData('entity_id'); 
			$realOrderId = (int)$order->getData('increment_id'); // last exported id			
                    ...
                    if (!empty($filename) || !empty($path)) {
                      if ($active == 'true') {
                        ...
                        if ($row && isset($row[0])) {
						  ...
						  file_put_contents($export_dir."/dataexport.xml", $xwoutput); // FOR TESTING MTU

					//	Mage::throwException(Mage::helper('export')->__('uffa' . $export_dir."/".$filename));
                          //Format filename
						  ...
						  $tdate = "Arquiste-" .Mage::getSingleton('core/date')->date('d')."-". Mage::getSingleton('core/date')->date('m')."-".Mage::getSingleton('core/date')->date('y') . "__";
						  $tdate = "";
						  /////////////////////////////////////
						  $filename = $id . "_" . $tdate  . $filename;
						  //////////////////////////////////////
						//uffaD:\SVN\Arquiste\public_html\shop/export/Worldship_Export.csv
                          ...
                          */
                          ...
                          if (!@file_put_contents($export_dir."/".$filename, $output)) {
                            //Mage::throwException(Mage::helper('export')->__('Could not open export file. Please check that we\'ve got write access to "'.$export_dir."/".$id."_".$filename.'"'));
							 ...
							 $filename = str_replace($id . "_", "", $filename);
...
//$pdf->save($export_dir."/pdf/rechnungen_packlisten_".$id.".pdf");
						...
						// here filename = uffaD:\SVN\Arquiste\public_html\shop/export/Worldship_Export.csv
