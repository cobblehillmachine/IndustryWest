...
class Idev_OneStepCheckout_AjaxController extends Mage_Core_Controller_Front_Action
{
    ...
    public function add_couponAction() /// swapped width other, /add_couponAction
    {
 ...
 $cert = Mage::getModel('ugiftcert/cert')->load($couponCode, 'cert_number');
        if ($cert->getId() && $cert->getStatus()=='A' && $cert->getBalance()>0) {
            ...
            $response['thirdparty'] = true;
            $response['message'] = Mage::helper('ugiftcert')->__("Gift certificate '%s' was applied to your order.", $cert->getCertNumber());
        } else {
        ...
        try {
            ...
            $response['success'] = false;
            $response['error'] = true;
            $response['message'] = $this->__('4Can not apply coupon code.');
        }
    ...
    public function add_giftcardAction2(){
